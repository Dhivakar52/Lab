...
interface PresidentLevelDetailProps {
  isOpen: boolean;
  onClose?: () => void;
  onRefresh?: () => void;
}
...
//Ganga
  ...
  const fetchNominationDetails = async () => {
    try {
      ...
      console.error("Failed to load President Level details", err);
    } finally {
      setLoading(false);
    }
  };
...
const generalScoreD = generalEntry?.GeneralJuryScore ?? null;
  ...
  const presStatusD =  presEntry?.PresidentStatus ?? "";
  const presCommentsD =presEntry?.PresidentComments ?? "";
  const presScoreD = presEntry?.PresidentScore ?? null;
  const presJuryApprovalsID = presEntry?.JuryApprovalsID ?? null;

  //Ganga

  if (loading) {
    return <div className="p-6">Loading President Level details...</div>;
  }
...
const openPopup = (type: "approve" | "reject") => {
    ...
    try {
      await axios.put(
         `${apiUrl}/api/presidentevaluation/${presJuryApprovalsID}`,
          {
            NominationID:data?.NominationID,
            IsPresidentApproved: true,
            PresidentComments: popupComments.trim(),
            PresidentScore:popupScore.trim() === "" ? null : Number(popupScore.trim()),
            PresidentID: userId,
            submittedBy: userId,
            active: true,
          },
    ...
    const onReject = async (presJuryApprovalsID: number) => {
    setLoading(true);
      try {
        ...
        await axios.put(
          `${apiUrl}/api/presidentevaluation/${presJuryApprovalsID}`,
          {
            ...
            PresidentComments: presCommentsD,
            PresidentScore:presScoreD,
            PresidentID: userId,
            submittedBy: userId,
            active: true,
          },
        ...
        setData((prev:any) =>
          prev.map((item:any) =>
            item.JuryApprovalsID === presJuryApprovalsID ? { ...item, Status: "Rejected" } : item
          )
        );
  ...
  const ActionButtons = () => {
    ...
    const RejectButton = () => (
      <button
        onClick={() => onReject(presJuryApprovalsID)}
        disabled={loading}
  ...
  <div className="min-h-screen bg-gray-100 flex flex-col">
    ...
    <div className="bg-white border-b border-gray-200 px-6 py-4 shrink-0 sticky top-0 z-20 shadow-sm">
      <h1 className="text-2xl font-semibold">President Level Details</h1>
    </div>
            ...
            <span
              className={`inline-flex px-3 py-1 rounded-full text-xs font-medium mt-1 ${
                presStatusD === "Pending"
                  ...
                  : presStatusD === "Approved"
                  ? "bg-green-100 text-green-800"
                  : presStatusD === "Rejected"
                  ? "bg-red-100 text-red-800"
                  : presStatusD === "Under Review"
          ...
          <div className="bg-white w-full max-w-xl p-6 rounded-lg shadow-lg">
            ...
            <div className="mt-4">
              <label className="text-sm font-medium">
                President Score <span className="text-red-600">*</span>
              </label>
...
export default PresidentLevelDetail;
