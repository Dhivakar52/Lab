...
export default function AddNomination() {
  ...
  const [nomineData, setNomineData] = useState<string[]>([]);
...
const formatted = res4.data.Users.map((u: any) => {
  const [name, email, company, department] = u.UserInfo
    .split(" - ")
    .map((x: string) => x.trim());

  return { UserID: u.UserID, name, email, company, department };
});
const uniqueDepartments: string[] = Array.from(
  new Set(formatted.map((u: any) => u.department))
);
const uniqueName: string[] = Array.from(
  new Set(formatted.map((u: any) => u.name))
);


setDepartments(uniqueDepartments);
setNomineData(uniqueName);
...
console.log("Entity Name", entitydropdown.data);
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  onChange={(selectedOption: any) =>
    setForm({ ...form, entityName: selectedOption?.value || "" })
  }
...
/>
  ...
  onChange={(selectedOption) =>
    setForm({ ...form, department: selectedOption?.value || "" })
  }
...
/>
  ...
  onChange={(selectedOption) =>
    setForm({ ...form, nomineeData: selectedOption?.value || "" })
  }
  placeholder="Select Department"
  className="w-full text-sm"
/>
